...
func (suite *TestSuiteTracker) TestIsVersionOlderByBuild(c *C) {
	...
	c.Assert(IsVersionOlderByBuild(v1, v2), Equals, true)
	c.Assert(IsVersionOlderByBuild(v2, v1), Equals, false)

	c.Assert(IsVersionOlderByBuild("foo/bar:v1.0-23", "foo/bar:v1.0-24"), Equals, true)
	c.Assert(IsVersionOlderByBuild("foo/bar:v1.0-24.5", "foo/bar:v1.0-24.6"), Equals, true)

	c.Assert(IsVersionOlderByBuild("foo/bar:v1.0-23", "foo/baz:v1.0-24"), Equals, false)

	c.Assert(IsVersionOlderByBuild("foo/bar:v1.0.23", "foo/baz:v1.0.24"), Equals, false)
	c.Assert(IsVersionOlderByBuild("foo/bar:v1.0.23", "foo/baz:v1.0.22"), Equals, false)
}
...
func (suite *TestSuiteTracker) TestContainers(c *C) {
	...
	for ch.Len() > 0 {
		...
		if ch.Len() > 0 {
			c.Assert(IsVersionOlderByBuild(popd.Image, (*ch)[0].Image), Equals, true)
		}
	}
}
